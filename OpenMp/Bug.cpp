...
//		grid->at(gridLoc.x).at(gridLoc.y).occupantBug = NULL;

		//assert(grid->at(suitableCells[chosenCell].x).at(suitableCells[chosenCell].y).hasBug == false);
		grid->at(suitableCells[chosenCell].x).at(suitableCells[chosenCell].y).hasBug = true;
//		grid->at(suitableCells[chosenCell].x).at(suitableCells[chosenCell].y).occupantBug = this;
...
const Vector2i Bug::getPosition() const {
	return gridLoc;
}
